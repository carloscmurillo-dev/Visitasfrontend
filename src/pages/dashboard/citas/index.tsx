...
import { CitasDrawer } from '../../../components/dashboard/citas/citas-drawer';
import { CitasListTable } from '../../../components/dashboard/citas/citas-list-table';
...
type TabValue = 'all' | 'PENDIENTE' | 'ACEPTADA' | 'ACORDADA' | 'RECHAZADA' ;
...
const applyFilters = (
  ...
  if (filters.query) {
    ...
    // name, email, etc.
    const containsQuery = (order.Name || '').toLowerCase().includes(filters.query.toLowerCase());
...
const MensajeList: NextPage = () => {
  ...
  const getMensajes = useCallback(async () => {
    try {
      ...
      const data = await  productApi.getMensajes('CITA      ');
          ...
          >
            ...
            >
              <TextField
                ...
                placeholder="Buscar por nombre del terapeuta..."
              />
            </Box>
            <TextField
              label="Ordenar por"
