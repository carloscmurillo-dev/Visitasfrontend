...
const CustomerList: NextPage = () => {
  ...
  const getProveedores = useCallback(async (h:string,m:string) => {
    try {
      //  alert(h + '  ' + m)
      const data = await productApi.getVisitasXFiltros(h?? '',m ?? '');
  ...
  /*  useEffect(
    ...
    [hospitalSelected,mesVisitaSelected]
  );  */
  ...
  return (
    <>
      <Head>
        <title>
          Dashboard: Control Visitas | Amimed Salud
        </title>
      </Head>
      <Box
        component="main"
      ...
      getRowId={(row) => row.Id}
        ...
        columns={[ { field: 'Id' },{ field: 'IdHospital' }, { field: 'DscHospital' }, { field: 'idPaciente' }
            , { field: 'NombrePaciente' }, { field: 'Cedula' },{ field: 'IdTerapeuta' }, { field: 'NombreTerapeuta' }, { field: 'IdMesVisita' }, { field: 'DscMesVisita' }
			...
			, { field: 'IdEstadoEquipo' }, { field: 'DscEstadoEquipo' }, { field: 'IdEvaluacionVisita' }, { field: 'DscEvaluacionVisitas' }
			, { field: 'ObservacionesClinicas' }, { field: 'ComentariosAdministrativos' }, { field: 'SharePoint' }
      , { field: 'EstadoPaciente' }, { field: 'DsEstadoPaciente' }, { field: 'AdherenciaUsoTratamiento' }, { field: 'DsAdherenciaUsoTratamiento' }
      , { field: 'CambEquiInsuTrata' }, { field: 'DsCambEquiInsuTrata' }, { field: 'CondTrataYParam' }, { field: 'DsCondTrataYParam' }
      , { field: 'SituaEspecYCoordi' }, { field: 'DsSituaEspecYCoordi' } ]}
        initialState={{
          pagination: {
            paginationModel: {
              pageSize: 5,
